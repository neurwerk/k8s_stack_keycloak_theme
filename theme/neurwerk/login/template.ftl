<#-- Adapted from Keycloak 26.7.2 keycloak.v2/login/template.ftl (Apache-2.0).
     Shared branding replaces the locale utility; native flow hooks are retained. -->
...
<head>
  ...
  <script>
    // Upstream workaround for https://bugzilla.mozilla.org/show_bug.cgi?id=1404468
  ...
  <div class="${properties.kcLoginContainer!}">
    ...
    <main class="${properties.kcLoginMain!}">
      <div class="${properties.kcLoginMainHeader!}">
        <#if pageId != 'login'>
          <div class="nw-page-brand">
            <span class="nw-company-name">${properties.companyName!'Example Company'}</span>
            <span class="nw-heading-logo" role="img" aria-label="Neurwerk"></span>
          </div>
        </#if>
        ...
        <#if auth?has_content && auth.showTryAnotherWayLink()>
          <form id="kc-select-try-another-way-form" action="${url.loginAction}" method="post" novalidate="novalidate">
            ...
            <a id="try-another-way" href="javascript:document.forms['kc-select-try-another-way-form'].requestSubmit()"
               class="${properties.kcButtonSecondaryClass} ${properties.kcButtonBlockClass} ${properties.kcMarginTopClass}">${msg("doTryAnotherWay")}</a>
          </form>
        </#if>
        <#if switchOrganizationEnabled?? && switchOrganizationEnabled>
          <form id="kc-switch-organization-form" action="${url.loginAction}" method="post" novalidate="novalidate">
            ...
            <a id="switch-organization" href="javascript:document.forms['kc-switch-organization-form'].requestSubmit()"
               class="${properties.kcButtonSecondaryClass} ${properties.kcButtonBlockClass} ${properties.kcMarginTopClass}">${msg("doSwitchOrganization")}</a>
          </form>
        </#if>
